...
Problem: Sensitive data leaks and security vulnerabilities in logs, files,
and text inputs cause major breaches. Manual review is slow and error-prone.

Solution: AI Secure Data Intelligence Platform - a real-time scanner that
ingests text, files, SQL, chat, and log data, detects 12+ threat patterns
using regex, scores risk with a weighted engine, and generates AI-powered
remediation insights using Claude API.

Technologies: Django 6 (REST API), React 18 (Vite), Claude Sonnet AI,
PyMuPDF, python-docx, django-cors-headers, Anthropic SDK.

Challenges: Handling multi-format file parsing, building a weighted risk
engine that avoids false positives, and designing AI prompts that return
specific non-generic security insights per finding type.
